...
INDIVIDUO reproducao(INDIVIDUO *populacao, int geracao){
	INDIVIDUO *populacaoCombinada, melhor, pai, mae, filho;
	int taxaDeElitismo = elitismo(populacao);

	// alta pressão seletiva tamanho dos pai * 7 (tamanho 150 * 7) , 15% de 1000.
	int tamanhoFilhos = taxaDeElitismo * 7;

 	// Aloca memoria para população que vai ser combinada usando o Modelo (μ+λ)-ES
	int tamanhoPopulacaoCombinada = tamanhoFilhos + taxaDeElitismo;
	populacaoCombinada = (INDIVIDUO *)malloc(tamanhoPopulacaoCombinada * sizeof(INDIVIDUO));

	melhor = populacao[0];

	// Copia a população atual dos 15% selecionados para a população que será combinada.
    for (int i = 0; i < taxaDeElitismo; i++) {
        populacaoCombinada[i] = populacao[i];
    }

	// Criando os filhos de tamanho pais * 7
	for(int i = 0; i < tamanhoFilhos; i++){
		...
		// Coloca os filhos na população combinada, onde já estão os 15% dos pais
		populacaoCombinada[taxaDeElitismo + i] = filho;
	}

	// Ordena toda a população combinada (pais + filhos juntos).
    qsort(populacaoCombinada, tamanhoPopulacaoCombinada, sizeof(INDIVIDUO), comparacao);

    // Seleciona os melhores indivíduos da população combinada para População que vai para proxima geração.
    // Copia de volta para a população principal 'populacao' com tamanho já pré definida.
    for (int i = 0; i < POPULACAO; i++) {
        populacao[i] = populacaoCombinada[i];
    }

	free(populacaoCombinada);
	return melhor;
}
